...
describe 'resource_artefacts::nexus_metrics' do
  ...
  context 'adds the consul-template files for the telegraf jolokia input' do
    ...
    it 'opens the Telegraf http listener port' do
      expect(chef_run).to create_firewall_rule('telegraf-http-listener').with(
        command: :allow,
        dest_port: 9090,
        direction: :in
      )
    ...
    it 'creates telegraf http template file in the consul-template template directory' do
      telegraf_metrics_username = node['nexus3']['users']['telegraf']['username']
      telegraf_metrics_password = node['nexus3']['users']['telegraf']['password']
      telegraf_http_template_content = <<~CONF
        # Telegraf Configuration
      ...
      expect(chef_run).to create_file('/etc/consul-template.d/templates/telegraf_http_inputs.ctmpl')
        .with_content(telegraf_http_template_content)
        .with(
          group: 'root',
          owner: 'root',
          mode: '0550'
    ...
    consul_template_telegraf_http_configuration_content = <<~CONF
      # This block defines the configuration for a template. Unlike other blocks,
      # this block may be specified multiple times to configure multiple templates.
      # It is also possible to configure templates via the CLI directly.
      template {
        ...
        # called the "Consul Template template". This option is required if not using
        # the `contents` option.
        source = "/etc/consul-template.d/templates/telegraf_http_inputs.ctmpl"

        # This is the destination path on disk where the source template will render.
        # If the parent directories do not exist, Consul Template will attempt to
        # create them, unless create_dest_dirs is false.
        destination = "/etc/telegraf/telegraf.d/inputs_http.conf"
        ...
        # return within 30s (configurable), and it must have a successful exit code.
        # Consul Template is not a replacement for a process monitor or init system.
        command = "/bin/bash -c 'chown telegraf:telegraf /etc/telegraf/telegraf.d/inputs_http.conf && systemctl restart telegraf'"
    ...
    CONF
    it 'creates telegraf_http_inputs.hcl in the consul-template template directory' do
      expect(chef_run).to create_file('/etc/consul-template.d/conf/telegraf_http_inputs.hcl')
        .with_content(consul_template_telegraf_http_configuration_content)
        .with(
          group: 'root',
          owner: 'root',
          mode: '0550'
